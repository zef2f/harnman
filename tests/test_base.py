...
def mock_fuzz_cmds():
    return {
        "harness1": "./fuzz_harness1",
        "harness2": "./fuzz_harness2"
    }

@pytest.fixture
def mock_cov_cmds():
    return {
        "harness1": "llvm-cov harness1",
        "harness2": "llvm-cov harness2"
    }
...
def test_expand_shell_substitutions():
    ...
    with patch('subprocess.check_output', return_value="test_output\n"):
        assert base._expand_shell_substitutions("prefix $(echo test) suffix") == "prefix test_output suffix"

    # Test multiple substitutions
    with patch('subprocess.check_output', side_effect=["one\n", "two\n"]):
        assert base._expand_shell_substitutions("$(echo one) middle $(echo two)") == "one middle two"
...
def test_get_cov_cmd(mock_read_config_field, mock_cov_cmds):
    ...
    for harn_name, expected_cmd in mock_cov_cmds.items():
        ...
        mock_read_config_field.assert_called_with(harn_name, "coverage.coverage_cmd")
