...
class TestHarnman(unittest.TestCase):
    ...
    @patch(
        "src.common.config_manager.list_config",
        return_value=["harness1", "harness2"],
    )
    ...
    @patch(
        "src.common.config_manager.list_config", side_effect=FileNotFoundError
    )
    ...
    @patch(
        "src.common.config_editor.read_config_field",
        return_value="./fuzz_harness1",
    )
    ...
    def test_print_fuzz_cmd_not_found(self, mock_read_config_field):
        ...
        self.assertEqual(
            error_output,
            "Error: Harness 'unknown_harness' not found or no fuzz command specified.",
        )

    @patch(
        "src.common.config_editor.read_config_field",
        return_value="clang -o harness1 harness1.c",
    )
    ...
    def test_print_build_cmd_not_found(self, mock_read_config_field):
        ...
        self.assertEqual(
            error_output,
            "Error: Harness 'unknown_harness' not found or no build command specified.",
        )

    @patch(
        "src.common.config_editor.read_config_field",
        return_value="llvm-cov harness1",
    )
    ...
    def test_print_cov_cmd_not_found(self, mock_read_config_field):
        ...
        self.assertEqual(
            error_output,
            "Error: Harness 'unknown_harness' not found or no coverage command specified.",
        )
